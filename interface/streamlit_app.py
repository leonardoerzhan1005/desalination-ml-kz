...
with st.expander("📊 🔬 Анализ кезеңдері (визуалды)"):
    ...
    if len(valid_df) == 0:
        ...
        st.success(f"📌 Модель болжаған сүзілген судың қысымы: **{predicted:.2f} бар**")

        if predicted < 3:
            st.warning("⚠️ Қысым тым төмен — сүзу нәтижесіз болуы мүмкін")
        elif predicted > 6:
            st.error("❗ Қысым тым жоғары — мембранаға қауіп!")
        else:
            st.info("✅ Қысым оптималды диапазонда")

        # 📐 Формуламен есептеу және визуализация
        st.subheader("📐 Формула арқылы есептелген сүзілген судың қысымы")
        pressure_formula = 0.0025 * example['тұздылық'] + 0.05 * example['температура'] - 0.3 * example['pH']
        st.write(f"Есептелген: **{pressure_formula:.2f} бар**")

        weights = {
            "Тұздылық әсері": 0.0025 * example['тұздылық'],
            "Температура әсері": 0.05 * example['температура'],
            "pH әсері (теріс)": -0.3 * example['pH']
        }
        ...
        fig_formula = px.bar(df_weights, x="Фактор", y="Қосқан үлесі (бар)",
                             title="Формула құрамындағы параметрлердің үлесі", text_auto=True)
        st.plotly_chart(fig_formula)
